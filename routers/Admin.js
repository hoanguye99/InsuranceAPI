...
router.post("/login/", async (req, res, next) => {
    ...
    let data = await login(userName,password,true);
    next(data);
});
...
router.get("/ins/",validateToken,  async (req, res, next) => {
    ...
    let data = await getListIns(userName,'ADMIN');
    next(data);
});
...
*/
router.put("/ins/:insId/apply/", validateToken,validateAdminRequired, async (req, res, next) => {
    let {userName} = req.payload;
    let {insId} = req.params;    
    let data = await applyIns(insId, userName);
    next(data);
});
